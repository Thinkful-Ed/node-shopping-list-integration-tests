...
describe('Recipes', function() {
  ...
  it('should add a recipe on POST', function() {
    const newRecipe = {name: 'pesto', ingredients:['basil', 'garlic', 'parmesan']};
  ...
  it('should update recipe on PUT', function() {
    const updateData = {
      name:'whatever',
      ingredients: ['some stuff', 'some more stuff', 'whatever else', 'shrugs']
    };
  ...
  it('should delete recipe on DELETE', function() {
    return chai.request(app).get('/recipes').then(function(res) {
      return chai.request(app).delete(`/recipes/${res.body[0].id}`).then(function(res) {
        res.should.have.status(204);
      });
    });
  });
